public static class duolingo
{
    public static readonly string[][] numberWords = new string[10][]
    {
        new[]
        {
            "Cero", "Uno", "Dos", "Tres", "Cuatro", "Cinco", "Seis", "Siete", "Ocho", "Nueve", "Diez", "Once", "Doce", "Trece", "Catorce", "Quince", "Dieciséis", "Diecisiete", "Dieciocho", "Diecinueve", "Veinte"
        },
        new[]
        {
            "Zero", "Uno", "Due", "Tre", "Quattro", "Cinque", "Sei", "Sette", "Otto", "Nove", "Dieci", "Undici", "Dodici", "Tredici", "Quattórdici", "Quindici", "Sedici", "Diciassette", "Diciotto", "Diciannove", "Venti"
        },
        new[]
        {
            "零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十"
        },
        new[]
        {
            "Zéro", "Un", "Deux", "Trois", "Quatre", "Cinq", "Six", "Sept", "Huit", "Neuf", "Dix", "Onze", "Douze", "Treize", "Quatorze", "Quinze", "Seize", "Dix-Sept", "Dix-Huit", "Dix-Neuf", "Vingt"
        },
        new[]
        {
            "Nul", "Een", "Twee", "Drie", "Vier", "Vyf", "Ses", "Sewe", "Agt", "Nege", "Tien", "Elf", "Twaalf", "Dertien", "Veertien", "Vyftien", "Sestien", "Sewentien", "Agtien", "Negantien", "Twintig"
        },
        new[]
        {
            "Sufuri", "Moja", "Mbili", "Tatu", "Nne", "Tano", "Sita", "Saba", "Nane", "Tisa", "Kumi", "Kumi na Moja", "Kumi na Mbili", "Kumi na Tatu", "Kumi na Nne", "Kumi na Tano", "Kumi na Sita", "Kumi na Saba", "Kumi na Nane", "Kumi na Tisa", "Ishirini"
        },
        new[]
        {
            "ゼロ", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十"
        },
        new[]
        {
            "영", "일", "이", "삼", "사", "오", "육", "칠", "팔", "구", "십", "십일", "십이", "십삼", "십사", "십오", "십육", "십칠", "십팔", "십구", "이십"
        },
        new[]
        {
            "Тэг", "Нэг", "Хоёр", "Гурав", "Дөрөв", "Тав", "Зургаа", "Долоо", "Найм", "Есөн", "Арав", "Арван нэг", "Арван Хоёр", "Арван Гурав", "Арван Дөрөв", "Арван таван", "Арван Зургаа", "Арван Долоо", "Арван Найм", "Арван Ес", "Хорин"
        },
        new[]
        {
            "ศูนย์", "หนึ่ง", "สอง", "สาม", "สี่", "ห้า", "หก", "เจ็ด", "แปด", "เก้า", "สิบ", "สิบเอ็ด", "สิบสอง", "สิบสาม", "สิบสี่", "สิบห้า", "สิบหก", "สิบเจ็ด", "สิบแปด", "สิบเก้า", "ยี่สิบ"
        }
    };
    public static readonly string[][] operatorWords = new string[10][]
    {
        new string[4]
        {
            "Más", "Menos", "Por", "Dividido por"
        },
        new string[4]
        {
            "Più", "Meno", "Per", "Diviso"
        },
        new string[4]
        {
            "加", "减", "乘", "除"
        },
        new string[4]
        {
            "Plus", "Moins", "Multiplié par", "Divisé par"
        },
        new string[4]
        {
            "Plus", "Minus", "Vermeeder", "Verdeel"
        },
        new string[4]
        {
            "Pamoja", "Kasoro", "Kuzidisha", "Gawanya"
        },
        new string[4]
        {
            "プラス", "マイナス", "かける", "割る"
        },
        new string[4]
        {
            "더하기", "빼기", "곱하기", "나누기"
        },
        new string[4]
        {
            "Дээрээс нь", "Хасах", "Үржүүлэх", "Хуваана"
        },
        new string[4]
        {
            "บวก", "ลบ", "คูณ", "หารด้วย"
        }
    };
}
